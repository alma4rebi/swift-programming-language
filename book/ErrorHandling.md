...
The `catch`{.code-voice} clauses don’t have to handle every possible
...
You use `try?`{.code-voice} to handle an error by converting it to an
...
A `defer`{.code-voice} statement defers execution until the current
