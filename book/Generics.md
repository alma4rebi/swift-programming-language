...
The `swapTwoInts(_:_:)`{.code-voice} function swaps the original value
...
For example, Swift’s `Dictionary`{.code-voice} type places a limitation
...
Any type that is `Equatable`{.code-voice} can be used safely with the
