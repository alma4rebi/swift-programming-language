...
The *bitwise NOT operator* (`~`{.code-voice}) inverts all bits in a
number:
...
Here’s how the bits inside an `Int8`{.code-voice} look for the number
`-4`{.code-voice}:
...
1.  `prefix`{.code-voice} `func`{.kt} - (`vector`{.vc}: `Vector2D`{.n})
    -&gt; `Vector2D`{.n} {
